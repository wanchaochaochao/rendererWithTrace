...
struct GPUAnimBitmap {
    ...
    void passive_motion( void (*f)(void*,int,int) ){
        passiveMotion = f;
    }

    void key_func( void(*f)(unsigned char,int,int,void*) ){
        keyFunc = f;
    }
    ...
    // static method used for glut callbacks
    static void passive_motion_func( int mx, int my ) {
        GPUAnimBitmap*   bitmap = *(get_bitmap_ptr());
        bitmap->passiveMotion( bitmap->dataBlock, mx, my );
    }
    ...
    static void Key(unsigned char key, int x, int y) {
        ...
        if ( bitmap->keyFunc ){
            bitmap->keyFunc( key, x, y, bitmap->dataBlock );
        }
    }
