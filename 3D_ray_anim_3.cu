...
struct Sphere{
    vec center, color;int num;int reflOrRefrac = 1;//0 反射, 1 折射
    float radius, reflRate = 1, refractionRate = 1.5;//光线的反射率和折射率
    Sphere(){}
    void setSphere(const vec &m_center, const float &m_radius, const vec &m_color, const int &m_reflOrRefrac){
        center=m_center, radius=m_radius, color=m_color, reflOrRefrac=m_reflOrRefrac;}
    //光线碰撞函数
    ...
    if(minDistance < INF){
        ...
        *startPoint += (*rayDir) * dir_norm_minDistance;
        ...
        if(indexHitPlane != -1){
            ...
            if(sphere[indexHitSphere].reflOrRefrac == 1){//折射
...
int main(void){
    ...
    while(fgets(buf,MAX_LINE,fid) != NULL){
        ...
        if(!strcmp(str, "Sphere")){//圆球
            for(int i = 0; i < 8; i++){inputData[i] = atof(strsep(&buff, " "));}
            data.cpu_sphere[sphereNum].setSphere(vec(inputData[0],inputData[1],inputData[2]), 
                inputData[3],vec(inputData[4],inputData[5],inputData[6]),inputData[7]);sphereNum++;}
